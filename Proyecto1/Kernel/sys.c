...
// Estructura para almacenar estadísticas I/O
struct io_stats {
    u64 rchar;        // Bytes leídos desde memoria
    u64 wchar;        // Bytes escritos a memoria
    u64 syscr;        // Operaciones de lectura
    u64 syscw;        // Operaciones de escritura
    u64 read_bytes;   // Bytes leídos desde disco
    u64 write_bytes;  // Bytes escritos a disco
};
...
// Estructura para almacenar estadísticas de syscalls
struct syscall_usage {
    unsigned long count;             // Conteo de llamadas
    struct timespec64 last_called;   // Última vez que se llamó
};
...
SYSCALL_DEFINE0(juferpr) {
    pr_info("Syscall sys_juferpr llamada correctamente\n");
    return 0;
}
...
SYSCALL_DEFINE1(capture_memory_snapshot, struct extended_sysinfo __user *, ext_info){
    ...
    e_info.si.freeswap = atomic_long_read(&nr_swap_pages); // Leer el valor subyacente

    // Inicializar active_memory en 0
    e_info.active_memory = 0;

    // Iterar sobre los nodos de memoria
    for_each_online_node(nid) {
        struct pglist_data *pgdat = NODE_DATA(nid);

        // Sumar las páginas activas (anónimas y en archivo) de cada nodo
        e_info.active_memory += node_page_state(pgdat, NR_ACTIVE_ANON);
        e_info.active_memory += node_page_state(pgdat, NR_ACTIVE_FILE);
    }

    // Convertir a KB
    e_info.active_memory <<= (PAGE_SHIFT - 10); // Convertir a KB
    e_info.si.totalswap <<= (PAGE_SHIFT - 10);  // Convertir a KB
    e_info.si.freeswap <<= (PAGE_SHIFT - 10);   // Convertir a KB
...
// Syscall para rastrear estadísticas
...
// Wrapper para inicializar rastreo
void track_syscall(int syscall_id) {
...
SYSCALL_DEFINE2(get_io_throttle, pid_t, pid, struct io_stats __user *, stats) {
    ...
    // Copia las estadísticas desde la estructura del kernel
    ...
    // Copia las estadísticas al espacio de usuario
