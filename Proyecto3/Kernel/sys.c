...
/*
	...
	talladoc: una variacion de calloc, solo que deja la memoria en reserved hastas ser accedida
	...
	------------------------------Desarrollado por Julio Fernandez XD ------------------------------
...
SYSCALL_DEFINE2(swap_info_syscall, unsigned long __user *, total, unsigned long __user *, free) {
    struct sysinfo si;

    // Obtener información del sistema
    si_meminfo(&si);

    unsigned long swap_total = si.totalswap << (PAGE_SHIFT - 10); // Convertir a KB
    unsigned long swap_free = si.freeswap << (PAGE_SHIFT - 10);   // Convertir a KB

    // Copiar resultados al espacio de usuario
    if (copy_to_user(total, &swap_total, sizeof(unsigned long)) ||
        copy_to_user(free, &swap_free, sizeof(unsigned long))) {
        return -EFAULT;
    }
